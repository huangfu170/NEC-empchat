...
class BartDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if knowl_hidden_states is not None:
            residual = hidden_states
            khidden_states, knowl_attn_weights, knowl_attn_present_key_value = self.knowl_attn(
                hidden_states=hidden_states,
            ...
            hidden_states=khidden_states+residual
            hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
            weight=self.know_gate(torch.cat([hidden_states,residual],dim=-1)).sigmoid()
...
class MyBartDecoder(BartPretrainedModel):
    ...
    def forward(
        ...
        """
            attention_mask为decoder_input_id的pad mask
       """
        ...
        if encoder_hidden_states.social_knowledge_encoder_outputs is not None:
            social_knowledge_outputs_mask=torch.ones(encoder_hidden_states.social_knowledge_encoder_outputs.size(0), encoder_hidden_states.social_knowledge_encoder_outputs.size(1),device=inputs_embeds.device)
        ...
        # Check if we have both social knowledge encoder outputs and valid attention masks
        if encoder_hidden_states.social_knowledge_encoder_outputs is not None:
